...
class Type_Tax {
	...
	public function title_placeholders( $title ) {
		...
		if ( $post_type_obj ) {
			...
			$post_title = esc_html__( $name . ' Title', APS_CONFIG['domain'] );
		} else {
			$post_title = esc_html__( 'Title', APS_CONFIG['domain'] );
		}

		// Apply a filter conditional modification.
		$title = apply_filters( 'aps_post_title_placeholders', $post_title );
	...
	public function update_messages( $messages ) {
		...
		foreach ( $post_types as $post_type => $post_object ) {
			...
			$messages[ $post_type ] = [
				...
				1  => sprintf(
					__( '%1s updated. <a href="%2s">View %3s</a>', APS_CONFIG['domain'] ), $post_object->labels->singular_name,
					esc_url( get_permalink( $post_ID ) ),
					$post_object->labels->singular_name
				),
				2  => __( 'Custom field updated.', APS_CONFIG['domain'] ),
				3  => __( 'Custom field deleted.', APS_CONFIG['domain'] ),
				4  => sprintf(
					__( '1%s updated.', APS_CONFIG['domain'] ),
					$post_object->labels->singular_name
				),
				5  => isset( $_GET['revision']) ? sprintf(
					__( '%1s restored to revision from %2s', APS_CONFIG['domain'] ),
					$post_object->labels->singular_name,
					wp_post_revision_title( (int) $_GET['revision'], false )
					) : false,
				6  => sprintf(
					__( '%1s published. <a href="%2s">View %3s</a>', APS_CONFIG['domain'] ),
					$post_object->labels->singular_name,
					esc_url( get_permalink( $post_ID ) ),
					$post_object->labels->singular_name
				),
				7  => sprintf(
					__( '%1s saved.', APS_CONFIG['domain'] ),
					$post_object->labels->singular_name
				),
				8  => sprintf(
					__( '%1s submitted. <a target="_blank" href="%2s">Preview %3s</a>', APS_CONFIG['domain'] ),
					$post_object->labels->singular_name,
				...
				9  => sprintf(
					__( '%1s scheduled for: <strong>%2s</strong>. <a target="_blank" href="%3s">Preview %4s</a>', APS_CONFIG['domain']  ),
					$post_object->labels->singular_name,
					date_i18n( __( 'M j, Y @ G:i', APS_CONFIG['domain'] ),
					strtotime( $post->post_date ) ),
					esc_url( get_permalink( $post_ID ) ),
					$post_object->labels->singular_name
				),
				10 => sprintf(
					__( '%1s draft updated. <a target="_blank" href="%2s">Preview %3s</a>', APS_CONFIG['domain']  ),
					$post_object->labels->singular_name,
					esc_url( add_query_arg( 'preview', 'true',
					get_permalink( $post_ID ) ) ),
					$post_object->labels->singular_name
