...
namespace PackStation\Classes\Users;
...
class User_Avatars extends Classes\Base {
	...
	public function capability() {
		...
		add_settings_field(
			'aps_user_avatars_caps',
			__( 'Avatar Upload Permission',	APS_CONFIG['domain'] ),
			[ $this, 'capability_field' ],
			'discussion',
			'avatars',
			[ esc_html__( 'Only allow users with file upload capabilities to upload local avatars (Authors and above).', APS_CONFIG['domain'] ) ]
		);
	...
	public function capability_field( $args ) {
		...
		$option = get_option( 'aps_user_avatars_caps' );

		$html = '<p><input type="checkbox" id="aps_user_avatars_caps" name="aps_user_avatars_caps" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="aps_user_avatars_caps"> ' . $args[0] . '</label></p>';
	...
	public function get_avatar( $avatar = '', $id_or_email, $size = 48, $default = '', $alt = false ) {
		...
		$local_avatars = get_user_meta( $user_id, 'aps_user_avatar', true );
		...
		if ( empty( $local_avatars[$size] ) ) {
			...
			update_user_meta( $user_id, 'aps_user_avatar', $local_avatars );
		...
		return apply_filters( 'aps_user_avatar', $avatar );
	}
	...
	public function user_avatar_form( $profileuser ) {
		...
		?>
		<h2><?php _e( 'User Avatar', APS_CONFIG['domain'] ); ?></h2>

		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="basic-user-avatar"><?php _e( 'Upload or Delete', APS_CONFIG['domain'] ); ?></label></th>
					<td style="width: 50px;" valign="top">
						<?php echo get_avatar( $profileuser->ID ); ?>
					</td>
					<td>
					<?php
					$options = get_option( 'aps_user_avatars_caps' );

					if ( empty( $options ) || current_user_can( 'upload_files' ) ) {

						// Nonce security.
						wp_nonce_field( 'aps_user_avatar_nonce', '_aps_user_avatar_nonce', false );

						// File upload input.
						echo '<input type="file" name="basic-user-avatar" id="basic-local-avatar" /><br />';

						if ( empty( $profileuser->aps_user_avatar ) ) {
							echo '<span class="description">' . __( 'No local avatar is set. Use the upload field to add a local avatar.', APS_CONFIG['domain'] ) . '</span>';

						} else {
							echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" /> ' . __( 'Delete local avatar', APS_CONFIG['domain'] ) . '<br />';
							echo '<span class="description">' . __( 'Replace the local avatar by uploading a new avatar or erase the current avatar by checking the delete option.', APS_CONFIG['domain'] ) . '</span>';
						}

					} else {

						if ( empty( $profileuser->aps_user_avatar ) ) {
							echo '<span class="description">' . __( 'You do not have permission to upload an avatar.', APS_CONFIG['domain'] ) . '</span>';

						} else {
							echo '<span class="description">' . __( 'You do not have media management permissions. To change your local avatar, contact the site administrator.', APS_CONFIG['domain'] ) . '</span>';
						}
					}
					?>
					</td>
	...
	public function edit_user_profile_update( $user_id ) {
		...
		if ( ! isset( $_POST['_aps_user_avatar_nonce'] ) || ! wp_verify_nonce( $_POST['_aps_user_avatar_nonce'], 'aps_user_avatar_nonce' ) ) {
			return;
		}
		...
		if ( ! empty( $_FILES['basic-user-avatar']['name'] ) ) {
			...
			update_user_meta( $user_id, 'aps_user_avatar', [ 'full' => $avatar['url'] ] );
	...
	public function bbpress_user_profile() {
		...
		echo '<div>';
		echo '<label for="basic-local-avatar">' . __( 'Avatar', APS_CONFIG['domain'] ) . '</label>';
		echo '<fieldset class="bbp-form avatar">';
				...
				if ( empty( $options ) || current_user_can( 'upload_files' ) ) {
					...
					if ( empty( $profileuser->aps_user_avatar ) ) {
						echo '<span class="description" style="margin-left:0;">' . __( 'No local avatar is set. Use the upload field to add a local avatar.', APS_CONFIG['domain'] ) . '</span>';

					} else {
						echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" style="width:auto" /> ' . __( 'Delete local avatar', APS_CONFIG['domain'] ) . '<br />';
						echo '<span class="description" style="margin-left:0;">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', APS_CONFIG['domain'] ) . '</span>';
					}

				} else {

					if ( empty( $profileuser->aps_user_avatar ) ) {
						echo '<span class="description" style="margin-left:0;">' . __( 'You do not have permission to upload an avatar.', APS_CONFIG['domain'] ) . '</span>';

					} else {
						echo '<span class="description" style="margin-left:0;">' . __( 'You do not have media management permissions. To change your local avatar, contact the site administrator.', APS_CONFIG['domain'] ) . '</span>';
					}
				}
	...
	public function avatar_default() {
		...
		$mystery = esc_url( APS_URL . 'assets/images/mystery.png' );
		$blank   = esc_url( APS_URL . 'assets/images/blank.png' );
	...
	public function get_avatar_defaults( $defaults = [] ) {
		...
		$defaults = [
			'mystery' => esc_url( APS_URL . 'assets/images/mystery.png' ),
			'generic' => esc_url( APS_URL . 'assets/images/generic.png' ),
			'yellow'  => esc_url( APS_URL . 'assets/images/yellow.png' ),
			'pink'    => esc_url( APS_URL . 'assets/images/pink.png' ),
			'blue'    => esc_url( APS_URL . 'assets/images/blue.png' ),
			'violet'  => esc_url( APS_URL . 'assets/images/violet.png' ),
			'green'   => esc_url( APS_URL . 'assets/images/green.png' ),
			'orange'  => esc_url( APS_URL . 'assets/images/orange.png' ),
			'blank'   => esc_url( APS_URL . 'assets/images/blank.png' )
		];

		// Return avatar types.
		return apply_filters( 'aps_get_avatar_defaults', $defaults );
	}
	...
	public function avatar_defaults( $options = [] ) {
		...
		$options = [
			$defaults['mystery'] => __( 'Mystery', APS_CONFIG['domain'] ),
			$defaults['generic'] => __( 'Generic', APS_CONFIG['domain'] ),
			$defaults['blank']   => __( 'Blank', APS_CONFIG['domain'] )
		];

		// Return new avatar options.
		return apply_filters( 'aps_avatar_defaults', $options );
	}
	...
	public function avatar_delete( $user_id ) {
		...
		$old_avatars = get_user_meta( $user_id, 'aps_user_avatar', true );
		$upload_path = wp_upload_dir();
		...
		delete_user_meta( $user_id, 'aps_user_avatar' );
	}
