...
class Plugins {
	...
	public function acf() {
		 ...
		 */
		if ( ! defined( 'APS_USE_ACF' ) ) {
			define( 'APS_USE_ACF', true );
		}
		 ...
		 * if the APS_USE_ACF constant is defined as true.
		 *
		 * @since 1.0.0
		 */
		if ( true == APS_USE_ACF && ! ( PackStation\active_acf() || PackStation\active_acf_pro() ) ) {
			$acf = new ACF;
		} else {
			$acf = null;
		}
	...
	public function acf_extended() {
		 ...
		 * @var   boolean Default is true.
		 */
		if ( ! defined( 'APS_USE_ACFE' ) ) {
			define( 'APS_USE_ACFE', true );
		}
		 ...
		 * APS_USE_ACFE constant is defined as true.
		 *
		 * @since 1.0.0
		 */
		if ( true == APS_USE_ACFE && PackStation\acf_ready() &&
			! ( PackStation\active_acfe() || PackStation\active_acfe_pro() ) ) {
			include_once( APS_PATH . 'includes/vendor/acf-extended/acf-extended.php' );
	...
	public function save_acf_json( $path ) {
		...
		$path = APS_PATH . 'includes/settings/acf-json';
	...
	public function load_acf_json( $paths ) {
		...
		unset( $paths[0] );
		$paths[] = APS_PATH . 'includes/settings/acf-json';
