...
namespace PackStation\Classes\Activate;
...
class Activate {
	...
	public function get_row_notice() {
		...
		if ( ! Classes\php()->version() ) {
			add_action( 'after_plugin_row_' . APS_BASENAME, [ $this, 'row_notice' ], 5, 3 );
		}
	}
	...
	public function row_notice( $plugin_file, $plugin_data, $status ) {
		...
		?>
		<style>
			.plugins tr[data-plugin='<?php echo APS_BASENAME; ?>'] th,
			.plugins tr[data-plugin='<?php echo APS_BASENAME; ?>'] td {
				box-shadow: none;
			}

			<?php if ( isset( $plugin_data['update'] ) && ! empty( $plugin_data['update'] ) ) : ?>

				.plugins tr.<?php echo APS_DOMAIN; ?>-plugin-tr td {
					box-shadow: none ! important;
				}

				.plugins tr.<?php echo APS_DOMAIN; ?>-plugin-tr .update-message {
					margin-bottom: 0;
				}
		...
		<tr id="plugin-php-notice" class="plugin-update-tr active <?php echo APS_DOMAIN; ?>-plugin-tr">
			<td colspan="<?php echo $colspan; ?>" class="plugin-update colspanchange">
				<div class="update-message notice inline notice-error notice-alt">
					<?php echo sprintf(
						'<p>%s %s %s %s %s %s</p>',
						__( 'Functionality of the', APS_DOMAIN ),
						APS_NAME,
						__( 'plugin has been disabled because it requires PHP version', APS_DOMAIN ),
						Classes\php()->minimum(),
						__( 'or greater. Your system is running PHP version', APS_DOMAIN ),
						phpversion()
					); ?>
				</div>
			</td>
