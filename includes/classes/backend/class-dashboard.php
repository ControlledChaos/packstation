...
class Dashboard extends Classes\Base {
	...
	function at_glance_svg( $colors = [] ) {
		...
		return apply_filters( 'aps_glance_svg', $colors );
	}
	...
	public function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'aps-svg-icon-colors', APS_URL . 'assets/js/svg-icon-colors.min.js', [ 'jquery' ], '', true );
	}
	...
	public function at_glance_end() {
		...
		echo sprintf(
			'<p>%s %s</p>',
			__( 'Your website is running PHP version', APS_DOMAIN ),
			phpversion()
		);
	}
