...
class Users extends Classes\Base {
	...
	public function __construct() {
		 ...
		 *
		 * If `APS_ALLOW_ADMIN_COLOR_PICKER` is set to false.
		 * This can be defined in the system config file.
		 */
		if ( defined( 'APS_ALLOW_ADMIN_COLOR_PICKER' ) && false == APS_ALLOW_ADMIN_COLOR_PICKER ) {
	...
	public function admin_enqueue_scripts() {
		...
		if ( 'profile.php' == $pagenow || 'user-edit.php' == $pagenow ) {
			wp_enqueue_script(
				...
				APS_URL . 'assets/js/user-bio' . $suffix . '.js',
				[ 'jquery' ],
				false,
				true
			);
	...
	public function menus_personal_data() {
		...
		$submenu['users.php'][25] = [
			__( 'Export Data', APS_CONFIG['domain'] ),
		...
		$submenu['users.php'][30] = [
			__( 'Erase Data', APS_CONFIG['domain'] ),
	...
	public function profile_editor( $user ) {
		...
		ob_start();
		?>
		<h2><?php _e( 'User Details', APS_CONFIG['domain'] ); ?></h2>

		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="description"><?php _e( 'Biographical Info', APS_CONFIG['domain'] ); ?></label></th>
					<td>
						<?php
						...
						?>
						<p class="description"><?php _e( 'Share a little biographical information to fill out your profile. This may be shown publicly.', APS_CONFIG['domain'] ); ?></p>
					</td>
				</tr>
			</tbody>
		</table>
